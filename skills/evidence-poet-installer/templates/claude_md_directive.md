<!-- Evidence Poet-DIRECTIVE-START -->
## Frontend design standard · Evidence Poet

This project's frontend implementation **must** follow the Evidence Poet Design System. The canonical spec is at [`.claude/design.md`](.claude/design.md) (installed by the `evidence-poet-installer` skill).
...
The Evidence Poet spec carries a `version` field (semantic versioning, `MAJOR.MINOR.PATCH`, e.g. `1.0.0`) in its §0 JSON block. When doing **any frontend work**, Claude should:

1. Read the `version` field from this project's [`.claude/design.md`](.claude/design.md).
2. If the global skill is installed (`~/.claude/skills/evidence-poet-installer/reference/design.md` exists), read its `version` field too.
3. Compare the two versions **numerically component-by-component** (not lexicographically — `1.10.0` is newer than `1.9.0`). If the skill's version is **newer**, tell the user once per session: "Evidence Poet spec has a newer version available (v<old> → v<new>). Run the `evidence-poet-installer` skill to update." Then continue with the current (older) project version.
