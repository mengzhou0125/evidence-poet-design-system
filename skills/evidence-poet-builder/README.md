...
content-review HTML — in Evidence Poet, this skill gives you the canonical spec, the right reference
implementation to copy from, and the anti-patterns to avoid.

Evidence Poet's character, in one line: *academic journal × architecture magazine — mono labels for
...
anti-patterns (4 Evidence Poet guardrails + user-strict rules) → propose architecture before coding
...
- Does not generate finished SVG diagrams → [`svg-diagram-skill`](https://github.com/mengzhou0125/svg-diagram-skill) (separate repo · pluggable spec · Evidence Poet default)
- Does not produce review HTML at depth (Scenario D defers to → [`html-review-skill`](https://github.com/mengzhou0125/html-review-skill), separate repo · pluggable spec · Evidence Poet default)
...
- `references/spec.md` — canonical token spec (mirror of installer's `reference/design.md`)
- `references/application-scenarios.md` — the 5 scenarios (A–E) + hybrid + 6th-scenario workflow
- `references/anti-patterns.md` — Evidence Poet guardrails + user-strict rules + extension governance

## Where this fits

Lifecycle: **install → build → verify**. This is the **build (generalist)** stage —
construct in Evidence Poet across surfaces. For two surfaces, depth-specialist skills (in
separate repos · pluggable spec · Evidence Poet default) do better:
