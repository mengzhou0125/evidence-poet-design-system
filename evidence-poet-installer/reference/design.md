...
**Versioning**: the `version` field uses semantic versioning (`MAJOR.MINOR.PATCH`, e.g. `1.0.0` → `1.0.1` → `1.1.0` → `2.0.0`) and **must be bumped on every meaningful change** to this spec. Downstream consumers (the `evidence-poet-installer` skill, projects with `.claude/design.md`) compare this field to detect drift and auto-update.
